...
\begin{frame}
    ...
    To solve more complex equations we use \emp{fixed point theory}
...
\begin{frame}
    ...
    \begin{itemize}
        \item write $T x$ instead of $T(x)$ and 
            \vspace{0.4em}
        \item call $T$ an \navy{operator} rather than a function
    \end{itemize}
...
\begin{frame}
    ...
    \begin{algorithm}[H]
      \DontPrintSemicolon
      fix $x_0$ and $k=0$ \;
      \While{some stopping condition fails}
      {
...
\begin{frame}
    ...
    We seek a \navy{root} of $h$, which is an $x^*$ such that $h(x^*)=0$
